...
export class Diary {
    ...
    mood: string;
    tags: string[];
    constructor(id: string, title: string, content: string, audioPath: string, createTime: number, isPinned: boolean = false, mood: string = '', tags: string[] = []) {
        this.id = id;
        this.title = title;
    ...
    static fromJson(json: DiaryJson): Diary {
        return new Diary(json.id, json.title, json.content, json.audioPath, json.createTime, json.isPinned ?? false, json.mood ?? '', json.tags ?? []);
    }
    toJson(): DiaryJson {
        return {
            id: this.id,
